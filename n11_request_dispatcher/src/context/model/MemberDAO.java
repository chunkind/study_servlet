...
public class MemberDAO {
	...
	public void registerMember(MemberDTO dto) throws SQLException {
		...
		try {
			conn = getConn();
			String query = "INSERT INTO member_ex VALUES(?,?,?,?)";
	...
	public MemberDTO findMemberById(String id) throws SQLException {
		...
		try {
			String query = "select * from member_ex where id=?";
			ps = conn.prepareStatement(query);
			ps.setString(1, id);
			rs = ps.executeQuery();
	...
	public ArrayList<MemberDTO> findMemberByAddress(String address) throws SQLException {
		...
		try {
			conn = getConn();
			String query = "SELECT * FROM member_ex WHERE address =?";
			ps = conn.prepareStatement(query);
			ps.setString(1, address);
			rs = ps.executeQuery();
			while (rs.next()) {
				list.add(new MemberDTO(rs.getString(1), rs.getString(2), rs.getString(3), address));
			}
		} finally {
			closeAll(rs, ps, conn);
		}
	...
	/*
	 * public static void main(String[] args) throws Exception{ // 단위테스트 한 다음,
	 * main은 지우겠다. MemberDAO dao = MemberDAO.getInstance();
